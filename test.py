...
class Solution:
    def minmumNumberOfHost(self , n, startEnd):
        # write code here
        startEnd.sort(key=lambda a: a[0])
        res = 1
        m = PriorityQueue()
        m.put((startEnd[0][1], startEnd[0]))
        for i in range(1, n):
            top = m.get()[1]
            if startEnd[i][0] < top[1]:
                m.put((top[1], top))
            m.put((startEnd[i][1], startEnd[i]))
            res = max(res, m.qsize())
        return res

s = Solution()
print(s.minmumNumberOfHost(10,[[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647],[2147483646,2147483647]]))