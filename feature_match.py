...
# 读取图片并转换成黑白图
im1 = cv2.imread('./data_color/5ms-3.bmp', cv2.IMREAD_COLOR)  # trainImage
im2 = cv2.imread('./data_color/5ms-4.bmp', cv2.IMREAD_COLOR)  # queryImage
im1 = cv2.resize(im1, dsize=None, fx=0.1, fy=0.1, interpolation=cv2.INTER_LINEAR)
im2 = cv2.resize(im2, dsize=None, fx=0.1, fy=0.1, interpolation=cv2.INTER_LINEAR)
# 直方图归一化，应对白色的头盔
