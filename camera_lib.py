...
def deviceLinkNotify(connectArg, linkInfo):
    if ( EVType.offLine == connectArg.contents.m_event ):
        print("camera_lib has off line, userInfo [%s]" %(c_char_p(linkInfo).value))
    elif ( EVType.onLine == connectArg.contents.m_event ):
        print("camera_lib has on line, userInfo [%s]" %(c_char_p(linkInfo).value))
...
def openCamera(camera):
    ...
    if ( nRet != 0 ):
        print("camera_lib connect fail!")
        return -1
    else:
        print("camera_lib connect success.")
...
def closeCamera(camera):
    ...
    if ( nRet != 0 ):
        print("disConnect camera_lib fail!")
        return -1
...
def demo():    
    ...
    nRet = streamSource.contents.getFrame(streamSource, byref(frame), c_uint(10000))
    if ( nRet != 0 ):
